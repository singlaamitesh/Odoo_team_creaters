...
router.get('/profile', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    // Get user's average rating and total ratings count
    const ratingResult = await db.get(`
      SELECT 
        (SELECT AVG(rating) FROM ratings WHERE userId = ?) as avg_rating,
        (SELECT COUNT(*) FROM ratings WHERE userId = ?) as total_ratings
